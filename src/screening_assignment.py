...
`--risk_cells_all` writes models/risk_cells_all.csv instead (see `risk_cells_all`).
"""
...
RISK_CELLS_ALL = os.path.join("models", "risk_cells_all.csv")
...
def risk_cells_all(net, df_test, assignment):
    """
    Cells of the observed covariates plus the unobserved parents of CRC, with
    p_crc = p(CRC | all of them) and p_crc_obs = p(CRC | observed covariates).
    Depression, Anxiety and SES are d-separated from CRC given its parents, so
    adding them would split cells without changing p_crc.
    """
    obs = observed_covariates(net)
    cols = obs + [c for c in net.get_parent_ids("CRC") if c not in obs]
    cells = enumerate_cells(df_test, cols)
    p = np.zeros(len(cells))
    for i, row in enumerate(cells[cols].to_dict(orient="records")):
        net.clear_all_evidence()
        for key, value in row.items():
            net.set_evidence(key, str(value))
        net.update_beliefs()
        p[i] = float(net.get_node_value("CRC")[1])
    cells["p_crc"] = p
    cells[obs] = cells[obs].astype(str)
    obs_p = assignment[obs + ["p_crc"]].astype({c: str for c in obs})
    return cells.merge(obs_p.rename(columns={"p_crc": "p_crc_obs"}), on=obs,
...
if __name__ == "__main__":
    ...
    if "--risk_cells_all" in sys.argv[1:]:
        full = risk_cells_all(net, df_test, pd.read_csv(OUT_FILE))
        full.to_csv(RISK_CELLS_ALL, index=False)
        print(f"{len(full)} all-covariate cells, {full['n'].sum():,} individuals; "
              f"missing p_crc_obs: {int(full['p_crc_obs'].isna().sum())}; written: {RISK_CELLS_ALL}")
        sys.exit(0)
